...
@muladd begin
    ...
    @inline function get_covariant_vector(index, contravariant_vectors, i, j, element)
        sign = 3 - 2 * index
        return SVector(
            -sign * contravariant_vectors[index, 2, i, j, element],
            sign * contravariant_vectors[index, 1, i, j, element],
        )
    end


    @inline function strong_form_kernel!(
        du,
        u,
        element,
        mesh::Union{
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            normu =
                contravariant_flux1[1] / rho * u_covariant_1 +
                contravariant_flux2[1] / rho * u_covariant_2

            for ii in eachnode(dg)
                ## Density
                du[1, ii, j, element] =
                    du[1, ii, j, element] + derivative_split[ii, i] * contravariant_flux1[1]

                ## Potential Temperature: conservative
                du[4, ii, j, element] =
                    du[4, ii, j, element] +
                    0.5 * derivative_split[ii, i] * contravariant_flux1[4]
                ...
                du[2, ii, j, element] =
                    du[2, ii, j, element] + 0.5 * derivative_split[ii, i] * normu
            ...
            for jj in eachnode(dg)
                ## Density
                du[1, i, jj, element] =
                    du[1, i, jj, element] + derivative_split[jj, j] * contravariant_flux2[1]

                ## Potential Temperature: conservative
                du[4, i, jj, element] =
                    du[4, i, jj, element] +
                    0.5 * derivative_split[jj, j] * contravariant_flux2[4]
                ...
                ## Momentum: conservative
                du[3, i, jj, element] =
                    du[3, i, jj, element] + 0.5 * derivative_split[jj, j] * normu
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            for k in eachnode(dg)
                ...
                theta = u_node[4] / u_node[1]
                Ja11, Ja12 =
                    get_contravariant_vector(1, contravariant_vectors, k, j, element)
                contravariant_flux1 = Ja11 * flux1 + Ja12 * flux2
                Ja21, Ja22 =
                    get_contravariant_vector(2, contravariant_vectors, k, j, element)
                contravariant_flux2 = Ja21 * flux1 + Ja22 * flux2
            ...
            contravariant_flux2 = Ja21 * flux1 + Ja22 * flux2
            du[2, i, j, element] +=
                -contravariant_flux2[1] / rho * tmp2 + theta * tmp3 * Ja11
            ...
            for k in eachnode(dg)
                ...
                theta = u_node[4] / u_node[1]
                Ja11, Ja12 =
                    get_contravariant_vector(1, contravariant_vectors, i, k, element)
                contravariant_flux1 = Ja11 * flux1 + Ja12 * flux2
                Ja21, Ja22 =
                    get_contravariant_vector(2, contravariant_vectors, i, k, element)
                contravariant_flux2 = Ja21 * flux1 + Ja22 * flux2
            ...
            du[3, i, j, element] +=
                -contravariant_flux1[1] / rho * tmp1 + theta * tmp3 * Ja22
