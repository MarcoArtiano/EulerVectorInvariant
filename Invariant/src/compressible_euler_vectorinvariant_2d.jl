...
@inline function flux_surface_total(u_ll, u_rr, normal_direction::AbstractVector,
	equations::CompressibleEulerVectorInvariantEquations2D)
	...
	g2 = v2_ll * jump_v1 * normal_direction[2] -v2_ll * jump_v2 * normal_direction[1] + equations.c_p * theta_avg * (exner_rr - exner_ll) * normal_direction[1]
	g3 = v1_ll * jump_v2 * normal_direction[1] -v1_ll * jump_v1 * normal_direction[2] + equations.c_p * theta_avg * (exner_rr - exner_ll) * normal_direction[2]

	return SVector(f1, f2 + 0.5f0 * g2, f3 + 0.5f0 * g3, f4, 0), SVector(f1, f2 - 0.5f0 * g2, f3 - 0.5f0 * g3, f4, 0)
end
...
@inline function flux_surface_noncons(u_ll, u_rr, normal_direction::AbstractVector,
	...
	jump_v2 = v2_rr - v2_ll
        ...
        f1 = 0.0
	f2 = v2_ll * jump_v1 * normal_direction[2] -v2_ll * jump_v2 * normal_direction[1] + equations.c_p * theta_avg * (exner_rr - exner_ll) * normal_direction[1]
	f3 = v1_ll * jump_v2 * normal_direction[1] -v1_ll * jump_v1 * normal_direction[2] + equations.c_p * theta_avg * (exner_rr - exner_ll) * normal_direction[2]
	f4 = 0.0
	return SVector(f1, f2, f3, f4, 0)
end
...
@inline function flux_surface_total_upwind(u_ll, u_rr, normal_direction::AbstractVector,
	equations::CompressibleEulerVectorInvariantEquations2D)
	rho_ll, v1_ll, v2_ll, rho_theta_ll = u_ll
	rho_rr, v1_rr, v2_rr, rho_theta_rr = u_rr
	_, _, _, exner_ll = cons2primexner(u_ll, equations)
	...
	diss2 = c / (2 * rho_avg) * (rho_v_rr - rho_v_ll) * normal_direction[2] / norm(normal_direction)^2
    	## According to Kieran notes I should use the average of the momentum in the density and potential temperature fluxes?
    	f1 = rho_avg * 0.5f0 * (v_dot_n_ll + v_dot_n_rr)
	f2 = kin_avg * 0.5f0 * normal_direction[1] - 0 * diss1 - 0 * 0.5 * c_adv/rho_avg * (rho_rr * v1_rr - rho_ll * v1_ll) * norm(normal_direction)	
	f3 = kin_avg * 0.5f0 * normal_direction[2] - 0 * diss2 - 0 * 0.5 * c_adv/rho_avg * (rho_rr * v2_rr - rho_ll * v2_ll) * norm(normal_direction)
	...
	jump_v1 = v1_rr - v1_ll
	jump_v2 = v2_rr - v2_ll
	theta = (theta_ll + theta_rr)*0.5f0	
	g2 = v2_ll * jump_v1 * normal_direction[2] -v2_ll * jump_v2 * normal_direction[1] + equations.c_p * theta * (exner_rr - exner_ll) * normal_direction[1]
	g3 = v1_ll * jump_v2 * normal_direction[1] -v1_ll * jump_v1 * normal_direction[2] + equations.c_p * theta * (exner_rr - exner_ll) * normal_direction[2]
...
@inline function flux_invariant(u_ll, u_rr, normal_direction::AbstractVector,
	equations::CompressibleEulerVectorInvariantEquations2D)
	# Unpack left and right state
	rho_ll, v1_ll, v2_ll, rho_theta_ll = u_ll
	rho_rr, v1_rr, v2_rr, rho_theta_rr = u_rr
	...
	g2 = v2_ll * jump_v1 * normal_direction[2] -v2_ll * jump_v2 * normal_direction[1] + equations.c_p * theta_avg* (exner_rr - exner_ll) * normal_direction[1] +phi_jump * normal_direction[1]
	g3 = v1_ll * jump_v2 * normal_direction[1] -v1_ll * jump_v1 * normal_direction[2] + equations.c_p * theta_avg* (exner_rr - exner_ll) * normal_direction[2] + phi_jump * normal_direction[2]
	# Average each factor of products in flux
	rho_avg = 0.5f0 * (rho_ll + rho_rr)
	theta_avg = 0.5f0 * (theta_ll + theta_rr)
	kin_avg = 0.5f0 * (v1_rr * v1_rr + v2_rr * v2_rr + v1_ll * v1_ll + v2_ll * v2_ll)
	...
	return SVector(f1, f2 + 0.5f0 * g2, f3 + g3 * 0.5f0, f4, 0), SVector(f1, f2 - 0.5f0 * g2, f3 - g3 * 0.5f0, f4, 0)
end
...
@inline function flux_volume_noncons(u_ll, u_rr, normal_direction::AbstractVector,
	...
	phi_jump = phi_rr - phi_ll
    	f1 = 0.0
	f2 = v2_ll * jump_v1 * normal_direction[2] -v2_ll * jump_v2 * normal_direction[1] + equations.c_p * theta_avg* (exner_rr - exner_ll) * normal_direction[1] + phi_jump * normal_direction[1]
	f3 = v1_ll * jump_v2 * normal_direction[1] -v1_ll * jump_v1 * normal_direction[2] + equations.c_p * theta_avg* (exner_rr - exner_ll) * normal_direction[2] + phi_jump * normal_direction[2]
